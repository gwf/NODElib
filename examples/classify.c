...
int main(int argc, char **argv)
{
  ...
  OPTION opts[] = {
    ...
    { "-dim",       OPT_INT,    &dim,       "input dimensionality"         },
  ...
  if (!train) {
    fprintf(stderr, "classify: you must use the -train option\n");
    exit(0);
  }
   ...
   * outputs.
   */
  nn = nn_create("%d %d 1", dim, nhid);  /* in-hidden-out architecture. */
  nn_link(nn, "0 -d-> 1");   /* Inputs to hidden link. */
  ...
  if ((trainser = series_read_ascii(train)) == NULL) {
    fprintf(stderr, "classify: cannot open \"%s\"\n", train);
    exit(1);
  }
  
  trainser->x_width = dim;
  trainser->y_width = 1;
