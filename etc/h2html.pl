...
sub do_text {
  ...
  foreach(@txt) {
    ...
    s/\>/&gt;/g;
    if(/\\begin{verbatim} *\n*/) {
      s/\\begin{verbatim}/<PRE><TT><p>/g;
      $verb = 1;
    }
    if(/\\end{verbatim}/) {
      s/\\end{verbatim}/<\/TT><\/PRE><p>\n/g;
      $verb = 0;
    }
    s/\\begin{itemize}/<UL>/g;
    s/\\end{itemize}/<\/UL><p>/g;
    ...
    s/\\indent *(.*)/\n<p style=\"0px;padding-left:20px;\">\n\1\n<\/p><p>\n/g;
    s/\\item/<LI>/g;
    s/\\it/\\em/g;
    s/\\newline/\n<BR>\n/g;
    s/\\section{([^}]*)}/<h3>\1<\/h3><p>/g;
...
#    $munge =~ s/^([^\(]*)(\s|\*)+\w+\(.*/\1/g;
  ...
  "<p class=\"code\"><small><tt><b>$_</b></tt></small></p><p>";
}
...
sub do_typedefs {
  ...
  while(1) {
	...
	$i--;
      ...
      $com[$#com] =~ s/(.*)\*\/.*\n/\1\n/g;
      if($first == 0) {
        print OUTFILE "<h3>Type Declarations</h3><p>";
	print OUTFILE "The following types are defined in the header ";
	print OUTFILE "file <I>$inname</I>.<P>\n";
	$first = 1;
      }
      print OUTFILE "<h2><b><A NAME=\"$name\">$name</A></b></h2>";
      @com = (@com, "\n");
      print OUTFILE "<p class=\"code\"><font color=\#993333><PRE><TT>";
      foreach (@td) {
        chop($_);
        s/\/\*\\n\*\//<br>/g;
...
sub do_protos {
  ...
  while(1) {
	...
	$i--;
      ...
      $com[$#com] =~ s/(.*)\*\/.*\n/\1\n/g;
      if($first == 0) {
        print OUTFILE "<h3>Function Definitions</h3><p>";
...
sub do_macros {
  ...
  while(1) {
	...
	$i--;
      ...
      $com[$#com] =~ s/(.*)\*\/.*\n/\1\n/g;
      if($first == 0) {
        print OUTFILE "<h3>Macro Definitions</h3><p>";
...
sub do_extras {
  ...
  while(1) {
    ...
    $com[$#com] =~ s/(.*)\*\/.*\n/\1\n/g;
    if($first == 0) {
      print OUTFILE "<h3>Miscellaneous Items</h3><p>";
      $first = 1;
    }
    &do_text(*com);
    $cur = $end + 1;
...
<link rel="stylesheet" href="format.css" type="text/css" media="screen, print">
</HEAD>
<BODY>

<h1 class="title"><b>NODElib</b> Documentation<p>By Gary William Flake</p></h1>

<div id="right">
<div id="vmenu">
<h1>Main Navigation</h1>
<a href="http://flakenstein.net/">GWF Home</a>
<a href="http://flakenstein.net/nodelib/html/">NODElib</a>
<a href="http://mitpress.mit.edu/flake/">CBofN</a>
</a>
...
EOF
  ...
  print OUTFILE "<p><B>#include &lt;$inname&gt;</B><P><TT>\n";
  foreach(@protos) {
    if(/^\s*private/i) {
      next;
    }
